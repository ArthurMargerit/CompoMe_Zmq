...
Zmq_server_component_interface_in::Zmq_server_component_interface_in()
    : CompoMe::Link() {
...
void Zmq_server_component_interface_in::step() {
  Link::step();
  ...
  auto l_res = CompoMe::Tools::call(this->get_map_map_of_caller_stream(),
                                    std::string(buffer));
...
void Zmq_server_component_interface_in::connect() {
  Link::connect();
...
void Zmq_server_component_interface_in::disconnect() {
  Link::disconnect();
...
CompoMe::String Zmq_server_component_interface_in::get_bind_addr() const {
  return this->bind_addr;
}

void Zmq_server_component_interface_in::set_bind_addr(
    const CompoMe::String p_bind_addr) {
